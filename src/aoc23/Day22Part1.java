...
public class Day22Part1 {
	...
	private static void dropBrick(Brick brick, String[][][] grid, Map<String, Coordinates3D> brickLocationsMap) {
		int movesDownCtr = countAvailableMovesDown(brick, grid, brickLocationsMap, null);
		if (movesDownCtr <  1) {
			return;
		}
	...
	private static int countAvailableMovesDown(Brick brick, String[][][] grid,
								Map<String, Coordinates3D> brickLocationsMap, String brickIdToIgnore) {
		...
		while (true) {
			...
			for (Coordinates3D c : cubesToCheck) {
				...
				if (cellValue != null && !cellValue.substring(0, 6).equals(brickIdToIgnore)) {
					canMoveDownAnotherLayer = false;
					break;
				}
			}
	...
	private static long countBricksSafeToRemove(String[][][] grid, Map<String, Brick> bricksMap,
												Map<String, Coordinates3D> brickLocationsMap) {
		long count = 0;
		for (Brick brick : bricksMap.values()) {
			//get any neighboring bricks directly above this one
			Set<Brick> neighboringBricksAbove = getNeighborsAbove(grid, bricksMap, brick, brickLocationsMap);

			//check if any of the neighbors would be able to move down if this one didn't exist
			boolean canAnyNeighborsMove = false;
			for (Brick neighbor : neighboringBricksAbove) {
				if (countAvailableMovesDown(neighbor, grid, brickLocationsMap, brick.id) > 0) {
					canAnyNeighborsMove = true;
					break;
				}
			}
			if (!canAnyNeighborsMove) {
				count++;
			}
		}
	...
	private static Set<Brick> getNeighborsAbove(String[][][] grid, Map<String, Brick> bricksMap, Brick brick,
												Map<String, Coordinates3D> brickLocationsMap) {
		Coordinates3D topCube = brick.cubes[brick.cubes.length - 1];
		List<Coordinates3D> zCubes = //any other cube in the block sharing the z coord of the top one
				Arrays.stream(brick.cubes).filter(c -> c.z() == topCube.z()).toList();

		Coordinates3D firstCubeLoc = brickLocationsMap.get(brick.id);
		List<Coordinates3D> cubesToCheck = new ArrayList<>();
		for (Coordinates3D cu : zCubes) {
			//adjust their coords to their grid location coords
			cubesToCheck.add(new Coordinates3D(cu.x() + firstCubeLoc.x(), cu.y() + firstCubeLoc.y(), cu.z() + firstCubeLoc.z()));
		}

		Set<Brick> neighboringBricksAbove = new HashSet<>();
		for (Coordinates3D c : cubesToCheck) {
			int targetZ = c.z() + 1;
			if (targetZ < grid[0][0].length && //don't check higher than top of the grid
					grid[c.x()][c.y()][targetZ] != null) {
				String brickId = grid[c.x()][c.y()][targetZ].substring(0, 6);
				neighboringBricksAbove.add(bricksMap.get(brickId));
			}
		}

		return neighboringBricksAbove;
	}
}
