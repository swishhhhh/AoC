...
public class Day16Part2 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < grid[0].length; i++) {
			largest = Math.max(largest, getEnergizedTileCount(grid, new GridStep(new Coordinates(i, 0), Direction.SOUTH)));
			largest = Math.max(largest, getEnergizedTileCount(grid, new GridStep(new Coordinates(i, grid.length - 1), Direction.NORTH)));
		}

		//first and last columns
		for (int i = 0; i < grid.length; i++) {
			largest = Math.max(largest, getEnergizedTileCount(grid, new GridStep(new Coordinates(0, i), Direction.EAST)));
